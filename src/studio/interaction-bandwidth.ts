...
function assertSnapshot(snapshot: BearingRuntimeSnapshot): void {
  if (!Number.isFinite(snapshot.massKg) || snapshot.massKg <= 0) {
    throw new Error("Runtime Hand requires positive finite body mass");
  }
...
export function createRuntimeHandGrab(
  ...
  return {
    ...
    localPoint: inverseRotateVec3ByQuat(
      snapshot.rotation,
      subtractVec3(worldPoint, snapshot.position),
    ),
...
export function runtimeHandAnchorWorld(
...
export function runtimeHandForceWorld(
  ...
  const pointVelocity = addVec3(
    snapshot.linearVelocity,
    crossVec3(snapshot.angularVelocity, armWorld),
  );

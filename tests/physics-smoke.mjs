...
for (const cutBridge of [false, true]) {
  test(`Box3D lowering preserves compiled mass (${cutBridge ? "cut" : "intact"})`, async () => {
    ...
    try {
      ...
      const errors = Object.values(runtime.receipt.bodyMassErrorsKg);
      assert.equal(errors.length, plan.bodies.length);
      for (const error of errors) assert.ok(Math.abs(error) < 0.05, `mass delta ${error}`);
